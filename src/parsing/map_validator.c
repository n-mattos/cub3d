...
/*   Updated: 2025/09/24 16:08:17 by mschippe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	get_portals(int **map, t_portal_list **portals)
{
	...
	while (map[y] != NULL)
	{
		...
		while (map[y][x] != '\0')
		{
			if (map[y][x] != FLOOR && map[y][x] != WALL && map[y][x] != EMPTY && !is_player(map[y][x]))
			{
				found_tp = find_node(*portals, map[y][x]);
				if (!is_valid_point(get_portal_target(map, x, y)))
					return (free_list(portals), false);
				else if (!found_tp)
					append_node(portals, create_node(map[y][x], (t_point){x, y}, get_portal_target(map, x, y)));
				else if(found_tp->B[SOURCE].x == -1)
					update_node(found_tp, (t_point){x, y}, get_portal_target(map, x, y));
				else
					return (free_list(portals), false);
			}
			x++;

...
/*   Updated: 2025/10/07 16:15:03 by nmattos-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_minimap(t_data *d)
{
	...
	fill_circle(d->minimap, (t_point){MMAP_DIAM / 2, MMAP_DIAM / 2},
		MMAP_DIAM / 2, EMPTY_COLOR);
	y = 0;
	while (d->level->map[y])
	{
		x = 0;
		...
		{
			draw_minimap_tile(d, d->level->player, x, y);
			x++;
		}
		y++;
	}
	draw_circle_outline(d->minimap,
...
static void	draw_minimap_tile(t_data *d, t_playerdata *p, int x, int y)
{
	if ((MMAP_DIAM / 2 + (x - p->x) * d->rect <= MMAP_DIAM + d->rect
			&& MMAP_DIAM / 2 + (y - p->y) * d->rect <= MMAP_DIAM + d->rect)
		&& MMAP_DIAM / 2 + (x - p->x) * d->rect > 0 - d->rect
		&& MMAP_DIAM / 2 + (y - p->y) * d->rect > 0 - d->rect)
	{
		drawrectangle(
			d->minimap, (t_point){d->rect, d->rect},
			(t_point){MMAP_DIAM / 2 + (x - p->x) * d->rect,
			MMAP_DIAM / 2 + (y - p->y) * d->rect},
...
void	draw_minimap_rays(t_data *d, t_playerdata p, t_vect intersect, int x)
{
	if (x >= 0 && x <= (int)IMG_WIDTH - 1 && x % (100 / PERCENTAGE_RAYS) == 0)
	{
		drawline(
			...
			(t_point){MMAP_DIAM / 2 + (intersect.x - p.x) * d->rect,
			MMAP_DIAM / 2 + (intersect.y - p.y) * d->rect},
			RED);
	}
}
